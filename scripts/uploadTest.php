...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['text_file'])) {
    ...
    if ($file['type'] !== 'text/plain') {
        ...
        header("Location: ../uploadPageTest.php");
        exit();
    }
    ...
    $newCollectionName = trim($_POST['new_collection_name'] ?? '');

    if ($collection === 'new' && !empty($newCollectionName)) {
        // Create a new collection
        $stmt = $conn->prepare("INSERT INTO collections (collectionType, collectionName, userID) VALUES ('book', ?, ?)");
        $stmt->bind_param("si", $newCollectionName, $userID);
        if (!$stmt->execute()) {
            $_SESSION['upload_message'] = "Failed to create a new collection.";
            header("Location: ../uploadPageTest.php");
            exit();
        }
        ...
        $collectionID = $collection; // Use the selected collection
    }

    // Add the book to the selected or created collection
    if (!empty($collectionID)) {
        $stmt = $conn->prepare("INSERT INTO itemsInCollection (collectionID, itemID, positionID) VALUES (?, ?, ?)");
        $positionID = 0; // Default position
        $stmt->bind_param("iii", $collectionID, $textID, $positionID);
        if (!$stmt->execute()) {
            $_SESSION['upload_message'] = "Failed to add the book to the collection.";
            header("Location: ../uploadPageTest.php");
            exit();
        }
        $stmt->close();
